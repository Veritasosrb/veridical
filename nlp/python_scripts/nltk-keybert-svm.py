...
#   feature_name = sys.argv[1]
  ...
  for line in text:
    ...
    output = nltk.pos_tag(tokens, tagset = "universal")
    for item in output:
      if(item[1] in adj_tag):
        nltk_adj.append(item[0])
      elif(item[1] in verb_tag):
        nltk_verb.append(item[0])
  ...
  add_head_dict = True
  header = ['dict']
  if(os.path.exists(path+"\\Dictionaries\\dictionary.csv") == add_head_dict):
    add_head_dict=False
