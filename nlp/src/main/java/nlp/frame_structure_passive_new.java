...
public class frame_structure_passive_new {
	...
	private static final String REGEX2 = "(VBZ)";
	private static final String REGEX3 = "(VBN)";
	...
	// sentence
	public void frame_passive_new(String sl, String k1) throws Exception {
		...
		// indicates nsubj is present in sentence
			...
			if (m.find()) {
				...
				int a1, b1;
				String op = "\0";
				for (SortedMap.Entry<Integer, Integer> entry1 : map1.entrySet()) {
					a1 = entry1.getKey();
					b1 = entry1.getValue();
					System.out.println(a1 + " " + b1);
					//int cnt = 1;
					Iterator<SortedMap.Entry<Integer, Integer>> iterator = map2.entrySet().iterator();
					...
					while (iterator.hasNext()) {
						...
						//while (iterator1.hasNext()) {
							...
							if (a>a1&&b>b1) {
								...
								if (k2 == -1) {
									if (op == "\0") {
										op = k1.substring(0, entry.getValue() + 1);
									} else {
										op += k1.substring(0, entry.getValue() + 1);
									}
									op += ".";
									System.out.println(k1.substring(0, entry.getValue() + 1));
								} else {
									if (op == "\0") {
										op = k1.substring(k2 + 1, entry.getValue() + 1);
									} else {
										op += k1.substring(k2 + 1, entry.getValue() + 1);
									}
									op += ".";
									System.out.println(k1.substring(k2 + 1, entry.getValue() + 1));
								}
				...
				System.out.println("Output final "+op);
				...
				// matches second pattern consisting of nn+vb+nn to pos tags of the sentence to
