...
public class JiraAPIRequest {
	...
	public List<String> getAllCommitsFromJira(String jiraID) throws IOException {
		...
		Request request = new Request.Builder().url(jiraURL).get().addHeader("authorization", credential).build();
		...
		jiraStashEntity.getDetail().get(0).getRepositories().get(0).getCommits().stream().forEach(entry -> {
			commitList.add(entry.getId());
		});
		return commitList;
	}
}